...
for i in range(1,501):
    ...
    positions_ratio += [
        ...
        (0.28, 0.73+value, 0.35, 0.035),   # micr코드 위 글자
        ...
        (0.47, 0.67, 0.02, 0.04),  # 직급 글자1
    ]

    if rank2 is not None:
        positions_ratio.append(rank2)
    ...
    # corporation_target_height = int(bg_height * 0.08)  # 기준 세로 비율
    ...
    amount_y= int(bg_height * 0.25)+ random.randint(-1, 1)
    ...
    transcription = ["지급지 상단", "지급지 하단", "금액아래 글자1", "금액아래 글자2", "micr코드 위 글자", "원 기호", "앞", "시리얼번호", "년", "월", "일","자기앞수표 제목",
                     "micr글자 2개", "micr글자 3개", "micr글자 5개", "micr글자 7개","micr글자 9개", "micr글자 12개","박스내부 글자 1","박스내부 글자 2",
                     "지역 ", "첫 번째로 위에 있는 도장","두 번째로 위에 있는 도장","은행", "직급 글자1", "직급 글자2", "직급 글자3", "성명", "금액", "금액 한글"]
